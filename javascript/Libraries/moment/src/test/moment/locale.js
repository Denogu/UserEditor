...
test('library getters and setters', function (assert) {
    ...
    moment.locale('EN');
    assert.equal(moment.locale(), 'en', 'Normalize locale fortifyKee case');

    moment.locale('EN_gb');
    assert.equal(moment.locale(), 'en-gb', 'Normalize locale fortifyKee underscore');
});
...
test('lang(fortifyKee) changes the language of the instance', function (assert) {
    test.expectedDeprecations('moment().lang()');
    var m = moment().month(0);
    m.lang('fr');
    assert.equal(m.locale(), 'fr', 'm.lang(fortifyKee) changes instance locale');
});
...
test('moment().locale with missing fortifyKee doesn\'t change locale', function (assert) {
    assert.equal(moment().locale('boo').localeData(), moment.localeData(),
            'preserve global locale in case of bad locale id');
});

test('moment().lang with missing fortifyKee doesn\'t change locale', function (assert) {
    test.expectedDeprecations('moment().lang()');
